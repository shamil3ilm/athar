...
| SEC-21 | Policies are declarative data, versioned | `athar-detection::config::PolicyConfig` — JSON file at `<data-dir>/config/policies.json` sets `enabled`, `mode`, `fail_mode` per rule and thresholds per signal. Daemon loads on boot; malformed / missing → defaults. `PolicyDecision::policy_version` per rule. CEL D11 evaluator deferred to Stage 2. | SCAFFOLDED |
| SEC-22 | Policy starts in OBSERVE mode; promotion is explicit | All V0 policies run in OBSERVE. Shim's `Runtime::evaluate()` receives full decision (action/mode/reason_codes) synchronously via `__evaluate__` request flag — caller may act on `Decision::wouldRestrict()`, but `Decision::isEnforced()` returns false until a policy is promoted (Stage 2). | SCAFFOLDED |
| INV-4  | Critical-path deadline + fail-open on breach | `Athar\Runtime::evaluate($event, deadlineMs)`; hard deadline via socket read timeout; on timeout returns `Decision::failOpen(DEADLINE_EXCEEDED)` — caller always gets a Decision, never blocked. Verified by `shim/bin/evaluate-test.php` "unreachable daemon → fail-open" assertion. | SCAFFOLDED (V0 wire; ENFORCE mode config pending Stage 2) |
